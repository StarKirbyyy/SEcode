...
const PREFIX = "secode-stage22.";
const MARKER_FILE = ".secode-stage22-marker";
const MARKER_CONTENT = "SECODE_STAGE22_FIXTURE_V1\n";
...
async function create(): Promise<void> {
  ...
  await writeFile(
    ...
    `# Stage 22 真实 LongCat 验收边界

- 这是一次性系统临时工作区；所有项目文件必须位于本工作区。
- 使用端口 4337 运行后端，使用端口 4338 运行前端；代码、脚本、README 和验证必须一致。
...
void create().catch((error: unknown) => {
  process.stderr.write(`${error instanceof Error ? error.message : "stage22 fixture failed"}\n`);
  process.exitCode = 1;
});
